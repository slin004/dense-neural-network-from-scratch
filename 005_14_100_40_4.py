...
def model(m_x_train, m_y_train, y_classes, model_lr, epochs,
          batch=1,
          given_w=[], given_b=[], save_params=False, model_name=""):
    ...
    for epoch in range(epochs):
        ...
        if save_params:
            ...
            dw_to_save.append(layer_o.dw)
            db_to_save.append(layer_o.db)
            save_params_to_csv(dw_to_save, db_to_save, model_name)
...
if __name__ == "__main__":
    ...
    # model(x_train_real, y_train_real, n_classes, model_lr=0.01, epochs=1, batch=1
    # print(label.shape)
    model(X, label, n_classes, model_lr=0.01, epochs=1, batch=1
          , given_w=w, given_b=b, save_params=True, model_name="100-40-4")
